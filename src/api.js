...
import { priceOnOrAfter, computeReturns } from './lib/returns';
...
// Coverage gap: paper filings scanned to image that the pipeline can't parse
// yet. Surfaced in the UI so missing data is visible, not silent.
let _paperPending = null;
export function getCoverageGap() {
  return _paperPending;
}
...
export async function getTrades({ force = false } = {}) {
  ...
  if (snap) {
    ...
    _paperPending = snap.meta?.paperPending || null;
    return {
