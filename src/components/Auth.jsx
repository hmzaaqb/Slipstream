...
const inputStyle = (focused) => ({
  ...
  border: `1px solid ${focused ? 'rgba(255,47,160,0.55)' : 'rgba(255,255,255,0.13)'}`,
...
const pinkCta = {
  ...
  fontSize: 15,
  letterSpacing: '1.5px',
  boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.45),inset 0 -2px 4px rgba(0,0,0,0.2),0 14px 34px rgba(255,47,160,0.45)',
};
...
function Divider({ label }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 22 }}>
      ...
      <span style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '2px', color: 'rgba(243,241,248,0.35)' }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.1)' }} />
    </div>
  );
}
...
function Note({ error, info }) {
  ...
  return (
    <div style={{ marginTop: 14, fontFamily: FONT.mono, fontSize: 12, lineHeight: 1.6, letterSpacing: '0.5px', color: error ? COLOR.red : 'rgba(243,241,248,0.6)' }}>
      {error || info}
    </div>
  );
}
  ...
  const features = [
    {
      ...
      icon: (
        <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
          <rect x="1.5" y="9" width="3.4" height="6" rx="1" fill="#FF6FC4" /><rect x="6.3" y="4" width="3.4" height="11" rx="1" fill="#FF6FC4" /><rect x="11.1" y="6.5" width="3.4" height="8.5" rx="1" fill="#FF6FC4" />
        </svg>
      ),
    ...
    {
      tint: 'rgba(120,82,255,0.16)', border: 'rgba(120,82,255,0.45)',
      icon: (
        <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
          <circle cx="8" cy="11" r="2" fill="#B98CFF" /><path d="M4 7a5.5 5.5 0 0 1 8 0M2 4.5a9 9 0 0 1 12 0" stroke="#B98CFF" strokeWidth="1.4" strokeLinecap="round" />
        </svg>
      ),
      title: 'Live trade feed',
      sub: 'Every disclosure streamed the moment it drops',
    },
    {
      tint: 'rgba(46,229,166,0.13)', border: 'rgba(46,229,166,0.4)',
      icon: (
        <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
          <rect x="2" y="2" width="8" height="8" rx="2" stroke="#2EE5A6" strokeWidth="1.4" /><rect x="6" y="6" width="8" height="8" rx="2" fill="#06060b" stroke="#2EE5A6" strokeWidth="1.4" />
        </svg>
      ),
      title: 'Copy their trades',
      sub: 'Mirror any politician automatically via Alpaca',
    },
  ];
  ...
  return (
    <div style={{ ...screenWrap, padding: '24px 26px 36px' }}>
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 34 }}>
        <div style={{ width: 108, height: 108, borderRadius: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'floaty 5s ease-in-out infinite', ...glass('strong', { borderRadius: 32, border: '1px solid rgba(255,255,255,0.2)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.4),inset 0 -2px 5px rgba(0,0,0,0.3),0 22px 50px rgba(255,47,160,0.35)' }) }}>
          <Bolt width={46} height={58} gradId="wlt" style={{ filter: 'drop-shadow(0 5px 14px rgba(255,47,160,0.6))' }} />
        </div>
      </div>

      <h1 style={{ margin: '30px 0 0', textAlign: 'center', fontFamily: FONT.black, fontWeight: 900, fontSize: 52, lineHeight: 0.88, letterSpacing: '-2.4px', WebkitTextStroke: '0.6px rgba(255,255,255,0.18)', background: 'linear-gradient(100deg,#FFFFFF 28%,#FF8FCE 62%,#B98CFF 100%)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>Slipstream</h1>
      <div style={{ textAlign: 'center', fontFamily: FONT.mono, fontSize: 11, letterSpacing: '5px', color: 'rgba(243,241,248,0.45)', marginTop: 14 }}>CATCH THE CURRENT</div>

      <div style={{ textAlign: 'center', fontFamily: FONT.archivo, fontWeight: 600, fontSize: 16, lineHeight: 1.55, color: 'rgba(243,241,248,0.72)', margin: '26px auto 0', maxWidth: 330 }}>
        ...
        {features.map((f) => (
          <div key={f.title} className="lg" style={{ display: 'flex', alignItems: 'center', gap: 15, padding: '15px 17px', borderRadius: 20, ...glass('mid', { borderRadius: 20 }) }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: FONT.archivo, fontWeight: 600, fontSize: 12.5, color: 'rgba(243,241,248,0.55)', marginTop: 3 }}>{f.sub}</div>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={screenWrap}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 24 }}>
        <Bolt width={24} height={30} gradId="slt" style={{ filter: 'drop-shadow(0 4px 10px rgba(255,47,160,0.55))' }} />
        <span style={{ fontFamily: FONT.mono, fontSize: 11, letterSpacing: '4px', color: 'rgba(243,241,248,0.45)' }}>CREATE ACCOUNT</span>
      </div>
      <h1 style={{ margin: '14px 0 0', fontFamily: FONT.black, fontSize: 38, lineHeight: 0.95, letterSpacing: '-1.6px' }}>
        Join the<br />
        <span style={{ background: 'linear-gradient(100deg,#FF8FCE,#B98CFF)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>slipstream</span>
      </h1>
      <div style={{ fontFamily: FONT.archivo, fontWeight: 600, fontSize: 14, color: 'rgba(243,241,248,0.55)', marginTop: 12 }}>
      ...
      <div style={{ textAlign: 'center', fontFamily: FONT.archivo, fontWeight: 600, fontSize: 14, color: 'rgba(243,241,248,0.5)', marginTop: 20 }}>
        Already have an account? <span onClick={() => go('signin')} style={{ color: '#FF8FCE', fontWeight: 800, cursor: 'pointer' }}>Sign in</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={screenWrap}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 30 }}>
        <div style={{ width: 84, height: 84, borderRadius: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', ...glass('strong', { borderRadius: 26, border: '1px solid rgba(255,255,255,0.2)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.4),0 18px 40px rgba(255,47,160,0.3)' }) }}>
      ...
      <div style={{ textAlign: 'center', fontFamily: FONT.archivo, fontWeight: 600, fontSize: 14, color: 'rgba(243,241,248,0.55)', marginTop: 10 }}>The current kept moving while you were away.</div>
...
function YoureIn({ onEnter }) {
  return (
    <div style={{ ...screenWrap, alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 26 }}>
      <div style={{ width: 96, height: 96, borderRadius: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'floaty 4s ease-in-out infinite', background: 'linear-gradient(155deg,rgba(46,229,166,0.2),rgba(46,229,166,0.06))', backdropFilter: 'blur(26px)', WebkitBackdropFilter: 'blur(26px)', border: '1px solid rgba(46,229,166,0.45)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.35),0 20px 46px rgba(46,229,166,0.3)' }}>
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none"><path d="M4 12.5 L9.5 18 L20 6.5" stroke="#2EE5A6" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </div>
      <h1 style={{ margin: '28px 0 0', fontFamily: FONT.black, fontSize: 34, lineHeight: 1, letterSpacing: '-1.2px' }}>You&rsquo;re in.</h1>
      <div style={{ fontFamily: FONT.archivo, fontWeight: 600, fontSize: 15, color: 'rgba(243,241,248,0.6)', marginTop: 12, maxWidth: 280 }}>
