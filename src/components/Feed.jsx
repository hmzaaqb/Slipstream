  ...
  return (
    <div style={{ padding: '13px 13px 11px', borderRadius: 16, ...glass('soft', { borderRadius: 16 }) }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '1px', color: 'rgba(247,247,245,0.6)' }}>{s.name}</span>
      ...
      <div style={{ height: 3, borderRadius: 2, marginTop: 8, background: s.up ? 'linear-gradient(90deg,#42C989,rgba(66,201,137,0.2))' : 'linear-gradient(90deg,#F0646E,rgba(255,107,91,0.2))' }} />
    </div>
  );
}
  ...
  return (
    <div onClick={onClick} style={{ flex: 'none', width: 128, padding: 14, borderRadius: 18, cursor: 'pointer', ...glass('mid', { borderRadius: 18 }) }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 8 }}>
        ...
        <span style={{ fontFamily: FONT.mono, fontSize: 9, letterSpacing: '0.5px', color: 'rgba(247,247,245,0.5)' }}>{h.meta}</span>
      </div>
    </div>
  );
}
...
function TradeCard({ t, onSelectPolitician }) {
  ...
  return (
    <div style={{ borderRadius: 24, overflow: 'hidden', ...glass('card', { borderRadius: 24, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.24),0 14px 32px rgba(0,0,0,0.45)' }) }}>
      {t.large && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', background: 'linear-gradient(90deg,rgba(212,175,55,0.3),rgba(212,175,55,0.18))', borderBottom: '1px solid rgba(255,255,255,0.1)', fontFamily: FONT.black, fontSize: 11, letterSpacing: '1.5px', color: '#C9B8FF' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: 18 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
            ...
            <span style={{ fontFamily: FONT.archivo, fontWeight: 700, fontSize: 13, color: 'rgba(247,247,245,0.55)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.company}</span>
          </div>
          <div onClick={() => onSelectPolitician && onSelectPolitician(t.pol)} style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 11, cursor: onSelectPolitician ? 'pointer' : 'default' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: FONT.mono, fontSize: 9, letterSpacing: '0.5px', color: 'rgba(247,247,245,0.45)' }}>{t.polSub}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: FONT.mono, fontSize: 9, letterSpacing: '0.5px', color: 'rgba(247,247,245,0.4)', marginTop: 11 }}>
            <ClockIcon opacity={0.4} />
            {t.disclosed}
          </div>
          <div style={{ fontFamily: FONT.mono, fontSize: 9, letterSpacing: '0.5px', color: 'rgba(247,247,245,0.35)', marginTop: 5 }}>{t.traded}</div>
        </div>
...
export default function Feed({ sectors, hotItems, feedTrades, ticker, setTicker, period, ftype, fsize, onPick, onSelectPolitician }) {
  return (
    <div className="fade-in">
      <div style={{ fontFamily: FONT.mono, fontSize: 12, letterSpacing: '2px', color: 'rgba(247,247,245,0.5)', marginTop: 20 }}>SECTOR INTEL · 90D</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: FONT.black, fontSize: 14, letterSpacing: '1px', color: COLOR.pink }}>
          <Bolt width={14} height={17} gradId="hotblt" from="#D4AF37" to="#D4AF37" />
          HOT THIS MONTH
        </div>
        <span style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '1px', color: 'rgba(247,247,245,0.4)' }}>tap to filter</span>
      </div>
      ...
      <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 18, ...glass('soft', { borderRadius: 18 }) }}>
        ...
        <input
          ...
          style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontFamily: FONT.archivo, fontWeight: 600, fontSize: 14, color: '#F7F7F5' }}
        />
      </div>

      <div style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '2px', color: 'rgba(247,247,245,0.5)', marginTop: 18 }}>PERIOD</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16 }}>
        <span style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '1px', color: 'rgba(247,247,245,0.5)', width: 38, flex: 'none' }}>TYPE</span>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <Chips opts={TYPE_OPTS} current={ftype} group="ftype" onPick={onPick} />
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
        <span style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: '1px', color: 'rgba(247,247,245,0.5)', width: 38, flex: 'none' }}>SIZE</span>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      ...
      <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: FONT.mono, fontSize: 12, letterSpacing: '2px' }}>
        <span style={{ color: 'rgba(247,247,245,0.7)' }}>{feedTrades.length} TRADES · LIVE</span>
      ...
      <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 14 }}>
        {feedTrades.length === 0 && (
          <div style={{ padding: 30, textAlign: 'center', fontFamily: FONT.mono, fontSize: 12, color: 'rgba(247,247,245,0.4)' }}>No trades match these filters.</div>
        )}
